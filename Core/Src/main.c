...
/* USER CODE BEGIN PV */
volatile SystemState system_state = {BUTTON_NONE, 0, 0, 0, 0, 0, 0, NONE, OFF, IN_PROCESS, MENU};
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pins : Down_Pin OK_Pin Cancel_Pin */
  GPIO_InitStruct.Pin = Down_Pin|OK_Pin|Cancel_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  /*Configure GPIO pin : MOVE_SENSOR_Pin */
  GPIO_InitStruct.Pin = MOVE_SENSOR_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_PULLDOWN;
  HAL_GPIO_Init(MOVE_SENSOR_GPIO_Port, &GPIO_InitStruct);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (system_state.debounce_flag == 0) {
		system_state.debounce_flag = 1;

		if (GPIO_Pin == Up_Pin)
			system_state.last_button = BUTTON_UP;
		else if (GPIO_Pin == Down_Pin)
			system_state.last_button = BUTTON_DOWN;
		else if (GPIO_Pin == OK_Pin)
			system_state.last_button = BUTTON_OK;
		else if (GPIO_Pin == Cancel_Pin)
			system_state.last_button = BUTTON_CANCEL;
		else if (GPIO_Pin == MOVE_SENSOR_Pin)
			system_state.small_move = 1;
